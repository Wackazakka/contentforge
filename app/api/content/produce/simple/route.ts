...
${needImagePrompts ? `Also give each line an image prompt (English, max 30 words) for a PAPER-CUT COLLAGE ILLUSTRATION that fits the line: objects, food, decorations, places, season — in a palette of ${palette}. The setting is Norway (wooden houses, gardens, Nordic light) unless the occasion says otherwise (Oktoberfest: Bavarian). People ARE welcome as generic, stylised paper figures that suit the occasion (dancing guests, people in lederhosen and dirndl, a 17 May parade, children in costumes) — always at a distance or mid-shot with simplified faces, never close-up portraits. STRICT: never depict the specific people named in the sender's text (no "the couple", no "the birthday girl"), never text in the image. Also give "extra": 4 more image prompts in the same style for general mood scenes of this occasion.` : 'Set image_prompt to an empty string and "extra" to an empty list.'}
