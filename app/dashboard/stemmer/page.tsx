...
const KIND_LABEL: Record<string, string> = { video: 'Video', avatar: 'Avatar', radio: 'Radio', face: 'Ansikt' }
...
export default function StemmerPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!token) throw new Error('Du må være innlogget for å se stemmene.')
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-2" style={{ letterSpacing: '-0.02em' }}>Stemmer{antall.ansikt > 0 ? ' og ansikter' : ''}</h1>
      <p className="text-[var(--ink-soft,#4A443B)] max-w-2xl mb-8">
        Ekte mennesker du kan bruke i det du lager hos {tenantName || 'oss'}. Prisen gjelder per bruk og kommer i tillegg til
        selve produksjonen. Skuespilleren får betalt hver gang, og alt føres i en hovedbok begge parter kan se.
      </p>

      {loading && <p className="text-[var(--text-muted,#6B6358)]">Henter stemmene …</p>}
      ...
      {valgt && (
        <div className="mb-8 p-4 rounded-xl border flex flex-wrap items-center gap-x-4 gap-y-2" style={{ background: 'var(--ember-tint-bg)', borderColor: 'var(--ember-tint-border)' }}>
          <div className="flex-1 min-w-[240px] text-sm">
            <strong>{valgt.name}</strong> står valgt til din neste video. Gå til Oversikt, åpne det du vil lage video for, og start en ny — stemmen er satt når utkastet åpner.
          </div>
          <button onClick={() => router.push('/dashboard')} className="px-4 py-2 rounded-lg text-sm font-semibold text-[var(--on-ember)] bg-[var(--ember-deep)] hover:opacity-90">Til Oversikt</button>
          <button onClick={angre} className="text-sm text-[var(--text-muted,#6B6358)] hover:text-[var(--ink,#1C1A16)] underline">Angre valget</button>
        </div>
      )}

      {!loading && !error && actors.length === 0 && (
        <div className="rounded-xl border p-8 max-w-xl" style={{ ...kant, background: 'var(--paper-raised)' }}>
          <h2 className="font-semibold text-lg mb-2">Ingen skuespillerstemmer ennå</h2>
          <p className="text-[var(--ink-soft,#4A443B)]">
            Banken er tom foreløpig. Standardstemmene i verktøyet virker som før — de følger med og koster ikke noe ekstra.
          </p>
        </div>
      )}

      {visFilter && (
        <div className="flex gap-2 mb-6 flex-wrap" role="tablist" aria-label="Vis">
                ...
                <dl className="text-sm mt-1 grid grid-cols-2 gap-x-4 gap-y-0.5">
                  {Object.entries(a.prices).map(([k, v]) => (
                    <div key={k} className="flex justify-between gap-2">
                      <dt className="text-[var(--text-muted,#6B6358)]">{KIND_LABEL[k] || k}</dt>
                      <dd className="font-medium">{nok(v)}</dd>
                    </div>
                  ))}
                </dl>
                <p className="text-[11px] text-[var(--text-faint,#8A8175)]">per bruk</p>
                ...
                <div className="mt-auto pt-2 flex items-center gap-2 flex-wrap">
                  ...
                  {a.isDemo ? (
                    ...
                    <button onClick={() => (erValgt ? angre() : bruk(a))}
                      ...
                      {erValgt ? '✓ Valgt' : 'Bruk denne stemmen'}
                    </button>
                  )}
                  {a.hasCard && <Link href={`/stemme/${a.id}`} target="_blank" className="text-sm font-medium text-[var(--ember-deep)] hover:underline ml-auto">Visittkort →</Link>}
                </div>
              </div>
            </article>
          )
